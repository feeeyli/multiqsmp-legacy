...
export const Streamer = ({
	...
	const [favoriteStreamers, setFavoriteStreamers] = useLocalStorage<string[]>(
		"favoriteStreamers",
		[]
	);

	function toggleFavoriteStreamers() {
		if (favoriteStreamers.includes(streamer.twitchName)) {
			const index = favoriteStreamers.indexOf(streamer.twitchName);

			setFavoriteStreamers((old) => {
			...
			setFavoriteStreamers((old) => [...old, streamer.twitchName]);
		}
	}
	...
	return (
		<div className="relative">
			...
			>
				{isFavorite && <HeartFilledIcon className="text-mandy-400" />}
